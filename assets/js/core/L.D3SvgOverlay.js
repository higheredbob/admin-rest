...
L.D3SvgOverlay = (L.version < "1.0" ? L.Class : L.Layer).extend({
    ...
    onAdd: function (map) {
        ...
        this.projection = {
            ...
            applyLatLngToLayer: function (d, zoom) {
                zoom = _layer._undef(zoom) ? _layer._zoom : zoom;
                var projectedPoint = _layer.map.project(new L.LatLng(d[1], d[0]), zoom)._round();
                return projectedPoint._subtract(_layer._pixelOrigin);
            },
            /*
            tweenDash: function(zoom) {
                    this._twee = 0;
                    var x = this;
                    return function(t) {
                        ...
                        if (this._twee == 0) {
                            this._twee = 1;
                            zoom = _layer._undef(zoom) ? _layer._zoom : zoom;
                            var newCenter = _layer.map.project(new L.LatLng(p.x, p.y), zoom)._round();
                            _layer.map.panTo(newCenter, 10);
                        } else {
                            this._twee = 0;
                        }
                        return i(t);
            ...
            */
            toLine: function(geom) {
                //zoom = _layer._undef(zoom) ? _layer._zoom : zoom;
                const toline = d3.line()
                    .curve(d3.curveLinear)
                    .x((geom) => _layer.map.project(new L.LatLng(geom[1], geom[0])).x)
                    .y((geom) => _layer.map.project(new L.LatLng(geom[1], geom[0])).y);
                return this;
            },
            transitionTween: function(path, transitionPT){
                ...
                path.transition()
                    .duration(function(d) {
                        if(d.properties.ddur !== undefined){
            ...
            pP: function() {
                this.geo = d3.geoTransform({point: proPoint});
                this.d3Path = d3.geoPath().projection(this.geo);
                function proPoint(x, y) {
